          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '48px', alignItems: 'start' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '36px' }}>
                {rooms.map(r => (
                  <div key={r.id} onClick={() => setSelectedRoom(r.id)} style={{ padding: '18px 20px', border: selectedRoom === r.id ? '2px solid #C8882A' : '1px solid #C5D4E8', background: selectedRoom === r.id ? '#EEF3FA' : '#F4F7FC', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                    <div>
              ...
              <div style={{ marginBottom: '36px' }}><label style={lbl}>Guests</label><select value={guests} onChange={e => setGuests(Number(e.target.value))} style={{ ...inp, cursor: 'pointer' }}>{[1,2,3,4].map(n => <option key={n} value={n}>{n} {n===1?'guest':'guests'}</option>)}</select></div>
            ...
            <div style={{ background: '#0A1F3D', padding: '36px' }}>
              ...
              <p style={{ fontFamily: 'Georgia, serif', fontSize: '20px', fontWeight: '500', color: '#F4F7FC', marginBottom: '4px' }}>{room.name}</p>
              ...
              <div style={{ borderTop: '1px solid rgba(200,136,42,0.2)', paddingTop: '20px' }}>
                {[
                  ...
                  { label: 'Rate', value: `$${room.price} / night` },
                ].map(item => (
